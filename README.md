...
Why agents? -- Agents can scan data inventory, gather inputs from relevant stakeholders and prepare plan for execution for data hub team to enable correct access.
What you created -- The workflow is a sequential loop process where agent seeks complete requirements from requestor, necessary approvals from data owners and security implementations from the subject matter experts and then prepare a plan for access activation for the data hub team.

Demo -- The demo notebook provides key functionality such as requirements refinement, requirements compaction, access approval process

The Build -- used Kaggle notebooks and ADK to build the solution
Access activation process can also be automated by using Azure infrastructure API calls.
